...
const ScoreSlider: React.FC<{ label: string, value: number, onChange: (value: number) => void, highlightKey?: string }> = ({ label, value, onChange, highlightKey }) => (
...
export const EpicEditor: React.FC<EpicEditorProps> = ({ epic, onSave, onCancel, isNew, highlightSection }) => {
  const { t } = useLocale();
      ...
      <div ref={editorContainerRef} className="bg-[#F0F4F4] rounded-lg shadow-2xl w-full max-w-6xl h-[95vh] flex flex-col" onMouseDown={e => e.stopPropagation()}>
        ...
        <main className="flex-1 flex overflow-hidden p-4 gap-4">
          <div className="flex-[3] overflow-y-auto pr-2 space-y-4">
            ...
            <div data-highlight-key="description">
             ...
             <div data-highlight-key="attachments">
              ...
              <AttachmentsManager attachments={localEpic.attachments || []} onChange={(atts) => setLocalEpic(prev => ({...prev, attachments: atts}))} />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-4 bg-white/50 p-4 rounded-lg">
            ...
            <ScoreSlider label={t('impact')} value={localEpic.impact || 5} onChange={val => setLocalEpic(p => ({...p, impact: val}))} highlightKey="impact" />
            <ScoreSlider label={t('confidence')} value={localEpic.confidence || 5} onChange={val => setLocalEpic(p => ({...p, confidence: val}))} highlightKey="confidence" />
            <ScoreSlider label={t('ease')} value={localEpic.ease || 5} onChange={val => setLocalEpic(p => ({...p, ease: val}))} highlightKey="ease" />
