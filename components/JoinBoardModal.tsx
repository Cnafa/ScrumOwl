...
const JoinBoardModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" onClick={onClose}>
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md" onClick={e => e.stopPropagation()}>
                ...
                <main className="p-6">
                    <p>Form to enter a board invite code would be here.</p>
                </main>
                 <footer className="p-4 border-t bg-gray-50 flex justify-end gap-2">
                    ...
                    <button className="py-2 px-4 bg-[#486966] text-white rounded-md">Join</button>
                 </footer>
            </div>
        </div>
    );
